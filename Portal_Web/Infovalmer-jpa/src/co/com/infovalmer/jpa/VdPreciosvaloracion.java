...
/**
 * The persistent class for the vd_preciosvaloracion database table.
 * 
 */
@Entity
@Table(name = "vd_preciosvaloracion")
@NamedQueries({
		@NamedQuery(name = "VdPreciosvaloracion.findAll", query = "SELECT v FROM VdPreciosvaloracion v"),
		@NamedQuery(name = "VdPreciosvaloracion.findFilterDate", query = "SELECT v FROM VdPreciosvaloracion v WHERE v.id.iIdinstrumento = ?1 and v.id.dFechavaloracion BETWEEN ?2 AND ?3") })
public class VdPreciosvaloracion implements Serializable {
	private static final long serialVersionUID = 1L;

	@EmbeddedId
	private VdPreciosvaloracionPK id;
	...
	public VdPreciosvaloracion() {
	}

	public VdPreciosvaloracionPK getId() {
		return this.id;
	}

	public void setId(VdPreciosvaloracionPK id) {
		this.id = id;
	}
